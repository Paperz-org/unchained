...
class MainAppModelMeta(models.base.ModelBase):
    """Metaclass that automatically sets app_label to '_unchained_app' for all models"""
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        if "Meta" not in attrs:
            attrs["Meta"] = type("Meta", (), {"app_label": "_unchained_app"})
        elif not hasattr(attrs["Meta"], "app_label"):
            setattr(attrs["Meta"], "app_label", "_unchained_app")
        ...
        if not is_abstract:
            ...
            print(
                f"Registered model {model_class.__name__} in MainAppModelMeta.models_registry"
            )
...
class MainModel(models.Model, metaclass=MainAppModelMeta):
    """
    Base model class that automatically sets app_label to '_unchained_app'
